                ...
                rows.append(
                    {
                        ...
                        **platform_columns(data, sub_rule),
                        "Tests": (
                            ", ".join(
                                sanitize_csv_cell(test) for test in sub_rule.get("tests", [])
                            )
                            or "-"
                        ),
            ...
            rows.append(
                {
                    ...
                    **platform_columns(data, rule),
                    "Tests": (
                        ", ".join(
                            sanitize_csv_cell(test) for test in rule.get("tests", [])
                        )
                        or "-"
                    ),
...
def write_markdown(data: dict, json_path: Path, csv_path: Path, path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            *([f"| `{platform}` | Whether ACS coverage exists for this platform; blank when not covered. |" for platform in platforms]),
