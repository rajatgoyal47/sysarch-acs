                           ...
                           uint64_t buf_size)
{
    ...
    /* Set interrupt enable bit in MPAMF_ECR (restore in cleanup) */
    val_mpam_mmr_write(msc_idx, REG_MPAMF_ECR, mpamf_ecr_saved | (1 << ECR_ENABLE_INTEN_SHIFT));

    /* Prefill close to max so a small traffic burst overflows */
    val_mpam_mbwu_write_counter(msc_idx,
        val_mpam_mbwu_get_prefill_value(msc_idx, MBWU_LONG_CHECK),
        MBWU_LONG_CHECK);
