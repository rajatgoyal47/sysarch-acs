...
static void payload_pe_test_info_check(void *arg)
{
  ...
  for (i = 0; i < num_pe; i++) {
    ...
    if (test_buffer->x0 < PFDI_ACS_SUCCESS) {
      ...
      val_print(DEBUG, "on PE  %d", i);
    }

    if ((test_buffer->x1 != 0) || (test_buffer->x2 != 0) ||
        (test_buffer->x3 != 0) || (test_buffer->x4 != 0)) {
      val_print(ERROR, "\n       Registers X1-X4 are not zero:");
      val_print(ERROR, " x1=0x%llx", test_buffer->x1);
      val_print(ERROR, " x2=0x%llx", test_buffer->x2);
      val_print(ERROR, " x3=0x%llx", test_buffer->x3);
      val_print(ERROR, " x4=0x%llx", test_buffer->x4);
      val_print(ERROR, "\n       Failed on PE = %d", i);
      test_fail++;
    }
