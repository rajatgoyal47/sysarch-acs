  ...
  while (tbl_index < bdf_tbl_ptr->num_entries)
  {
      ...
      if (dp_type == RP || dp_type == iEP_RP)
      {
        ...
        */
        ...
        val_print(DEBUG, " Memory lim is  0x%llx", mem_lim);

        /* If Memory Limit is programmed with value less the Base, then Skip.*/
        if (mem_lim < mem_base) {
            val_print(DEBUG, "\n       No P memory on secondary side of the Bridge");
        ...
        */
        ...
        /**Check_2: Accessing out of P memory limit range must result in UR completion
         *
