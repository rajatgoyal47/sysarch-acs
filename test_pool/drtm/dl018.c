...
#define TEST_NUM   (ACS_DRTM_DL_TEST_NUM_BASE  +  18)
#define TEST_RULE  "R45470"
#define TEST_DESC  "Check Debug excp masked on boot PE    "
...
payload(uint32_t num_pe)
{
  ...
  /* verify debug exceptions (Process state D mask) are masked */
  if (((read_daif >> DRTM_DEBUG_EXECPTION_SHIFT_AIF) & DRTM_DEBUG_EXECPTION_MASK_AIF) !=
                                            DRTM_DEBUG_EXCP_MASKED) {
    val_print(ERROR, "\n       Debug exceptions are not masked on Boot PE");
